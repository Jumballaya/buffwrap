...
test("Slice shares memory with original buffer", () => {
  const buffer = new BufferWrap<TestStruct>({ ...config, capacity: 4 });
...
test("Invalid slices do not crash", () => {
  const buffer = new BufferWrap<TestStruct>({ ...config, capacity: 2 });

  expect(() => buffer.slice(2, 1)).not.toThrow();
  expect(() => buffer.slice(0, 0)).not.toThrow();

  const empty = buffer.slice(1, 1);
  expect([...empty.iterate()]).toStrictEqual([]);
});
