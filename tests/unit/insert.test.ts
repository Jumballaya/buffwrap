...
describe("BufferWrap [insert]", () => {
  test("Insert from ArrayBuffer", () => {
    ...
    const raw = buffer.getStrategy().getBuffer().slice(0, buffer.stride);
    buffer.insert(0, raw);
  ...
  test("Insert rejects incompatible BufferWrap", () => {
    ...
    const otherConfig = {
      ...
      struct: {
        a: {
          length: 1,
          type: cfg.struct.a.type,
        },
        b: {
          length: 3,
          type: cfg.struct.b.type,
        },
      },
      capacity: 1,
    };
    const other = new BufferWrap<TestStruct, ArrayBuffer>(otherConfig);
